...
compare_dives <- function(b1, b2) {
	...
	if(nrow(b1) > 0 & nrow(b2) > 0) {
		...
		out <- list(tag1 = b1$DeployID[1], tag2 = b2$DeployID[2], t1 = t1, t2_matched = t2, diff_times = diff_times, diff_durs = diff_durs, diff_deps = diff_deps, type1 = diff_b1_type, type2 = diff_b2_type)
	} else {
		warning("maybe tags don't overlap?")
		out <- NULL
	}
