...
datadir <- "~/Downloads/tmp/batch_downloaded_latest"
savedir <- "~/Downloads/tmp/batch_downloaded_latest"
...
# centered on cee (cst, cen)
pdf(file.path(savedir, "cee0912.pdf"))
...
# since sep 05 (second batch of tags)
for(i in 1:length(zcs)) {
  pdf(file.path(savedir, paste0(zcs[i], "-since05sep.pdf")))
